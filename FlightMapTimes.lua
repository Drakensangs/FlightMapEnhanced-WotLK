...
local function lSaveFlightTime(length, from, to, flightType)
    ...
    local map = FlightMapUtil.getFlightMap();
    if not map[from] then return; end
    ...
    local defaultTime = FLIGHTMAP_FLIGHTS[from]
                    and FLIGHTMAP_FLIGHTS[from].Flights
                    and FLIGHTMAP_FLIGHTS[from].Flights[to];
    if defaultTime and math.abs(defaultTime - length) < 1.0 then return; end

    -- Update the session map so the rest of this session sees the new time.
    map[from].Flights[to] = length;

    -- Also write to the savedvariable so it persists across sessions.
    local faction = UnitFactionGroup("player");
    local sv = FlightMapUtil.lGetOverrides(faction);
    if not sv[from]         then sv[from]         = {}; end
    if not sv[from].Flights then sv[from].Flights = {}; end
    sv[from].Flights[to] = length;
end
...
function FlightMapTimes_BeginFlight(duration, destination)
    ...
    if not FlightMapChar.Opts.useTimer then return; end
